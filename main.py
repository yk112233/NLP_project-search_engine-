...
process = ['开发者','学术论文','初学者','语言上','中英文','计算机','对模型','人民日报','浮点数','主题词','感兴趣','阿拉伯','标点符号','利用计算机','文件名','西班牙','程序员','高质量']
dabiaoofcf = ['支持向量机','决策树']
dabiaoofcl = ['神经网络']
dabiaoofmt = ['神经网络']
classofword = {}
class Document:
    def __init__(self, key, title, neirong, link, emotion, like, comment, collection):
        self.dict = {'key': key, 'title': title, 'neirong':neirong, 'link': link, 'emotion': emotion, 'like': like, 'comment':comment, 'collection':collection}
...
def text_preprocess(text):  
    ...
    text_list =[]
    for x in a:
        if x.flag == 'n' or x.flag == 'nz':
            text_list.append(x.word)
    text_array = np.unique(text_list)
    delete_indexs = []
    for i in range(len(text_array)):
        if text_array[i] in stopwords_Chinese:
            delete_indexs.append(i)

    np.delete(text_array, delete_indexs)
    return text_array
...
class Indexer:
    ...
    def index_writer(self, file_path):
        ...
        sheet1 = workbook.sheet_by_name('My Worksheet')
        for i in range(sheet1.nrows):
            key = sheet1.cell(i, 0).value
            title = sheet1.cell(i, 1).value
            neirong = sheet1.cell(i, 2).value
            link = sheet1.cell(i, 3).value
            emotion = sheet1.cell(i, 4).value
            like = sheet1.cell(i, 5).value
            comment = sheet1.cell(i, 6).value
            collection = sheet1.cell(i,7).value
            doc = Document(key, title, neirong, link, emotion, like, comment, collection)
    ...
    def index(self):
        ...
        for doc in self.doc_list:
            ...
            # 正排
            self.id_doc[key] = doc
            self.id_doc_len[key] = len(doc.get('neirong'))
            S += self.id_doc_len[key]
            # 倒排
            #term_list = (text_preprocess(doc.get('neirong'))).tolist()# 分词
            term_list = list(jieba.cut_for_search(doc.get('neirong')))
            ans.append(list(set(term_list)-set(stopwords_Chinese)))
            for t in term_list:
        ...
        for t in self.inverted:
            self.idf[t] = math.log10((doc_num - len(self.inverted[t]) + 0.5)  / (len(self.inverted[t])+0.5))
        idf = self.idf
        for k in sorted(idf, key=idf.__getitem__):
            print(k, idf[k])
        #print(self.inverted)
        ...
        for term in term_list:
            if term in self.index.inverted:
                for doc_id, fre in self.index.inverted[term].items():
                    if doc_id in tf_idf:
                        tf_idf[doc_id] += self.index.idf[term] * fre/self.index.id_doc_len[doc_id]*(2+1)/(fre/self.index.id_doc_len[doc_id] + 2*(1-0.75 + 0.75*self.index.id_doc_len[doc_id]/avgdl))

                        #tf_idf[doc_id] += (float(self.index.id_doc[doc_id].get('emotion'))+0.2)*self.index.idf[term] * fre/self.index.id_doc_len[doc_id]*(2+1)/(fre/self.index.id_doc_len[doc_id] + 2*(1-0.75 + 0.75*self.index.id_doc_len[doc_id]/avgdl))
                    else:
                        tf_idf[doc_id] = self.index.idf[term] * fre/self.index.id_doc_len[doc_id]*(2+1)/(fre/self.index.id_doc_len[doc_id] + 2*(1-0.75 + 0.75*self.index.id_doc_len[doc_id]/avgdl))
...
def search(query):
    ...
    load_wv = KeyedVectors.load('word_vector0')
    target = query
    target=check.auto_correct_sentence(query)
    print(query,target)
    flag = 0
    if target != query:
        flag = 1
    ans = []
    # classofword_key = list(classofwidord.keys())
    # classofword_value = list(classofword.values())
    # conscores = np.zeros(len(classofword_key))
    # id = 0
    # MAX = query.path_similarity(classofword_key[0])
    # if len(docs) == 0:
    #     for i in range(1,len(classofword_key)):
    #         conscores[i] = query.path_similarity(classofword_key[i])
    #         if MAX < conscores[i]:
    #             MAX = conscores[i]
    #             id = i
    #     query = classofword_key[id]
    #     print(query)
    #     docs = searcher.search(query)
    end = time.time()
    ...
    if flag == 0:
        ...
        for word in terms:
            if word in load_wv.vocab.keys():
                res = load_wv.similar_by_word(word,topn=30)
                for i in res:
                    if len(i[0]) > 2 and i[0] not in process:
                        ans.append(i[0])
    else:
        terms = list(jieba.cut_for_search(target))
        for word in terms:
            if word in load_wv.vocab.keys():
                res = load_wv.similar_by_word(word,topn=30)
                for i in res:
    ...
    ans = ans[:3]
    return render_template('search.html', docs=result, value=query, length=len(docs),target=target,flag=flag,ans=ans,time = round(end-start,6))
...
def highlight(docs, terms):
    ...
    for doc in docs:
        ...
        result.append((doc.get('link'), content, doc.get('emotion'), doc.get('like'), doc.get('comment'), doc.get('collection')))
    return result


index = Indexer("Excel_test4.xls")
classofword = index.inverted
