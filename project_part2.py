...
class Indexer:
    ...
    def index(self):
            ...
            for t in term_list:
                if t in self.inverted:
                    ...
                    if key not in self.inverted[t]:
                        self.inverted[t][key] = 1/len(term_list)
                    else:
                        self.inverted[t][key] += 1/len(term_list)
                else:
                    self.inverted[t] = {key: 1/len(term_list)}
        ...
        for t in self.inverted:
            self.idf[t] = math.log10(doc_num / (1+len(self.inverted[t])))
        ...
        for term in term_list:
            if term in self.index.inverted:
                for doc_id, fre in self.index.inverted[term].items():
                    if doc_id in tf_idf:
                        tf_idf[doc_id] += fre * self.index.idf[term]
                    else:
                        tf_idf[doc_id] =  fre * self.index.idf[term]
        # 排序
...
if __name__ == '__main__':
    ...
    print("0")
    searcher.search("深度学习 CNN 蜂巢")
    print("1")
    print(len(index.doc_list))