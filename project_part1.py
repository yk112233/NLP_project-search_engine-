...
from naive_bayes import emotion
...
def text_preprocess(text):
    ...
    text_array = list(jieba.cut_for_search(text))
    delete_indexs = []
...
if __name__ == '__main__':
    ...
    zzz = np.arange(1, 2)
    left = ['https://so.csdn.net/so/search/s.do?q=%E6%96%87%E6%9C%AC%E8%81%9A%E7%B1%BB&t=blog&platform=pc&p=',
            'https://so.csdn.net/so/search/s.do?q=%E5%88%86%E8%AF%8D&t=blog&platform=pc&p=',
            'https://so.csdn.net/so/search/s.do?q=%E6%9C%BA%E5%99%A8%E7%BF%BB%E8%AF%91&t=blog&platform=pc&p=',
            'https://so.csdn.net/so/search/s.do?q=NLP&t=blog&platform=pc&p=',
            'https://so.csdn.net/so/search/s.do?q=%E6%96%87%E6%9C%AC%E5%88%86%E7%B1%BB&t=blog&platform=pc&p=']
    right = ['&s=&tm=&v=&l=&u=&ft=','&s=&tm=&v=&l=&u=&ft=','&s=&tm=&v=&l=&u=&ft=','&s=&tm=&v=&l=&u=&ft=','&s=&tm=&v=&l=&u=&ft=']
    yk_cnt = 0
    for loop in range(4,5):
        for m in range(len(zzz)):
            url = left[loop] + str(zzz[m]) + right[loop]
            reg = r'(//blog.csdn.net/.+/article/details/[0-9].+)"' # 正则表达式
            reg_ques = re.compile(reg)  # 编译一下正则表达式，运行的更快
            travel_urls = reg_ques.findall(get_html(url))  # 匹配正则表达式
            for i in range(len(travel_urls)):
                str1 = travel_urls[i]
                pattern1 = re.compile(r'//blog.csdn.net/.+/article/details/[0-9]+')
                new_str = re.findall(pattern1, str1)[0]
                travel_urls[i] = new_str
            ...
            for i in range(len(travel_urls)):
                ...
                for item in travel_neirong:
                    ...
                    if len(line) > 1:
                        clause_text += line
                #qinggan = emotion(clause_text)
                clause_text = text_preprocess(clause_text)
                emotions.append(qinggan)
            ...
            for j in range(len(titles)-yk_cnt):
                ...
                worksheet.write(yk_cnt, 1, str(titles[yk_cnt]))
                worksheet.write(yk_cnt, 2, str(neirongs[yk_cnt]))
                worksheet.write(yk_cnt, 3, str(urls[yk_cnt]))
                worksheet.write(yk_cnt, 4, str(emotions[yk_cnt]))
